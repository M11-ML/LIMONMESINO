...
from data import (
    DIST, NAMES, DEFAULT_DEMAND, PRODUCT_SPLIT, CAPACITY,
    CANTON_POLY, CANTON_COLOR, CENTERS, MAP_SIZE, PROVINCE_OUTLINE,
)
...
if run:
    ...
    if total_demand == 0:
        ...
        # contorno general de la provincia (silueta tipo mapa político)
        outline = plt.Polygon(
            PROVINCE_OUTLINE, closed=True, facecolor="#e9edf2",
            edgecolor="#5a6b7a", linewidth=2.2, alpha=1, zorder=0,
        )
        ax.add_patch(outline)

        # cantones con la forma real (aproximada) de cada cantón
        for i in range(1, 9):
            poly = CANTON_POLY[i]
            patch = plt.Polygon(
                poly, closed=True, facecolor=CANTON_COLOR[i],
                edgecolor="#ffffff", linewidth=1.5, alpha=0.95, zorder=1,
            )
            ax.add_patch(patch)
            cx, cy = CENTERS[i]
            ax.annotate(
        ...
        for idx, r in enumerate(routes):
            offset = (idx % 5) * 1.5
            xs = [CENTERS[n][0] + offset for n in r]
            ys = [CENTERS[n][1] + offset for n in r]
            ax.plot(xs, ys, color="#d6001c", linewidth=2.2, alpha=0.8,
                    zorder=3, solid_capstyle="round")

        # depósito (CD)
        x0, y0 = CENTERS[0]
        ax.scatter(x0, y0, s=300, color="#ffffff", edgecolor="#d6001c",
                   linewidth=2, marker="*", zorder=5)
        ax.annotate("CD Cartago", (x0, y0), textcoords="offset points",
